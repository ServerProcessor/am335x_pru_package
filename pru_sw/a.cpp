...
int   DemSetPositionProfile(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode);
...
int DemoProfilePositionMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	...
	if(VCS_SetPositionProfile(p_DeviceHandle, p_usNodeId,2000,500,0, &p_rlErrorCode) == 0 )
	{
		LogError("VCS_SetProfilePosition", lResult, p_rlErrorCode);
		lResult = MMC_FAILED;
	}
	...
	else
	{
		...
		for(list<long>::iterator it = positionList.begin(); it !=positionList.end(); it++)
		{
			...
			if(VCS_MoveToPosition(p_DeviceHandle, p_usNodeId, 409600, 0, 1, &p_rlErrorCode) == 0)
			{
